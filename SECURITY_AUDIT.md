...
| T-14 | Strategy replay — reactivating an old strategy | CODED | `lib.rs:269-272`, `lib.rs:634` | — |
...
| T-30 | Rapid price movement between decision and executio | CODED | `lib.rs:631`, `lib.rs:335-336` | — |
...
| T-35 | Trade exceeding limits — max_trade_bps and cooldow | CODED | `lib.rs:312-318`, `state.rs:27-44` | — |
...
**2. The swap path's coverage is fork-gated.** Everything proving the CPI is
...
**3. Confidentiality is bounded by inference, not by cryptography.** T-9 is
...
**4. Several mitigations are described in the present tense and do not exist.**
The threat model claims jittered cadence, randomised size, CSP, SRI,
reproducible builds, published hashes, and `tx.jup.ag` submission. None are
implemented. Prose written as though shipped is the same failure as a comment
describing a guarantee the code does not provide.
