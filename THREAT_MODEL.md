...
| **T-27** | Sandwich attack on our swap | On-chain `min_amount_out` derived from Pyth at execution time and asserted after the CPI, bounded by `max_slippage_bps`. `tx.jup.ag` submission with a tip is **not** implemented. There is **no** separate post-swap deviation-band check — this row claimed one for a while and none existed. | **Partially mitigated.** `TradeIntent` is public before execution, so intent is visible. Short expiry windows reduce but do not eliminate exposure. Do not claim front-running resistance. |
