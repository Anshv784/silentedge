...
| **T-9** | **Threshold inference from public trades** | Each evaluation yields a `(price, action)` pair and each pair is an inequality; enough of them squeeze every threshold between tight bounds. **No mitigation is implemented.** Jittered cadence, randomised size (`ArcisRNG`) and threshold bands are all V2 candidates; written here in the present tense they read as shipped defences and are not. | **Inherent and unfixable.** Acting publicly reveals why you acted. Bounds tighten roughly logarithmically in the number of evaluations straddling a threshold — days, not years, for an active bot. Must be disclosed prominently. Full analysis: [`docs/what-is-private.md`](docs/what-is-private.md). |
| **T-38** | **`size_bps` was fully recoverable from a single trade** | `amount = balance × size_bps / 10_000`, with the balance public in the same transaction and `amount` revealed — one non-HOLD evaluation disclosed it **exactly**. **Resolved by moving it out of the encrypted struct** into `RiskLimits.size_bps`, where it is public and owner-editable. The strategy is now three encrypted integers, not four. | **Closed as a privacy claim.** The value is public, and is now described that way in the UI and the SDK. Side benefit: the evaluate circuit halved, 246,850 -> 114,762 bytes. |
...
| ID | Threat | Mitigation | Residual |
|----|--------|-----------|----------|
| **T-15** | Malicious frontend exfiltrates plaintext before encryption | The frontend is a genuine trust boundary here: it *sees* plaintext by necessity. Implemented: a strict CSP with a bounded `connect-src` (apps/web/next.config.mjs), no third-party scripts on the builder page, open source. **Not** implemented: SRI, reproducible builds, published hashes. | **Real and unavoidable.** A user who does not trust the served frontend must self-host. Disclose. |
...
| **T-27** | Sandwich attack on our swap | On-chain `min_amount_out` derived from Pyth at execution time and asserted after the CPI, bounded by `max_slippage_bps`. `tx.jup.ag` submission with a tip is **not** implemented. A decision-to-execution deviation band **is** now enforced on entries (`max_oracle_deviation_bps`): an authorization that would fill more than the band above the price it was decided at is refused. Exits are exempt — a stop fires because the price fell, and refusing it because the price fell further would disarm the vault's only downside control in the move it exists for. | **Partially mitigated.** `TradeIntent` is public before execution, so intent is visible. Short expiry windows reduce but do not eliminate exposure. Do not claim front-running resistance. |
